...
if 'data' not in st.session_state:
    st.session_state['data'] = pd.DataFrame(
        columns=['PLT','APTT','D.Dime','ALT','AST','CK','Urea','Cr','FERR2.1.ng.ml','Prediction','Label'])

# 在主页面上显示数据
st.header('老年噬血细胞综合征30天死亡风险预测')
...
# # 2. 选择需要归一化的列
# columns_to_normalize = ['Age', 'PT', 'CA199', 'LDH', 'CK']
# data_to_normalize = train_data[columns_to_normalize]
#
# # 3. 初始化MinMaxScaler并拟合训练数据
# scaler = MinMaxScaler(feature_range=(0, 1))
# scaler.fit(data_to_normalize)

# Input bar 1
a = st.sidebar.number_input('PLT', value=27)
b = st.sidebar.number_input('APTT', value=35.2)
c = st.sidebar.number_input('D.Dime',value=5.06)
d = st.sidebar.number_input('ALT', value=14)
e = st.sidebar.number_input('AST', value=21.2)
f = st.sidebar.number_input('CK', value=41)
g = st.sidebar.number_input('Urea', value=25.7)
h = st.sidebar.number_input('Cr', value=99.4)
i= st.sidebar.number_input('FERR2.1.ng.ml', value=7500)
...
if st.sidebar.button("Submit"):
    ...
    X = pd.DataFrame([[a, b, c, d, e,f,g,h,i]],
                     columns=['PLT','APTT','D.Dime','ALT','AST','CK','Urea','Cr','FERR2.1.ng.ml'])
    # X = X.replace(["Brown", "Blue"], [1, 0])
    trans = imputer.transform(X)
    ...
    # Output prediction
    st.text(f"基于随机森林算法的老年HLH患者30天死亡风险: {str(result_prob_pos)}%")
    # st.text({str(shap_values[0])})

    # 创建一个新的DataFrame来存储用户输入的数据
    new_data = pd.DataFrame([[X.iat[0, 0], X.iat[0, 1], X.iat[0, 2], X.iat[0, 3], X.iat[0, 4], X.iat[0, 5], X.iat[0, 6], X.iat[0, 7], X.iat[0, 8], result_prob_pos / 100, None]],
...
if uploaded_file is not None:
    ...
    # 检查是否所有必需的列都存在
    missing_cols = [col for col in ['PLT','APTT','D.Dime','ALT','AST','CK','Urea','Cr','FERR2.1.ng.ml'] if
    ...
    if missing_cols:
        ...
        for _, row in df.iterrows():
            ...
            new_data = pd.DataFrame([[X.iat[0, 0], X.iat[0, 1], X.iat[0, 2], X.iat[0, 3], X.iat[0, 4], X.iat[0, 5], X.iat[0, 6], X.iat[0, 7], X.iat[0, 8], result_prob, label]],
                                    columns=st.session_state['data'].columns)
            st.session_state['data'] = pd.concat([st.session_state['data'], new_data], ignore_index=True)
